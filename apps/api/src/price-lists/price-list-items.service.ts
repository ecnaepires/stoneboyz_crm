...
export class PriceListItemsService {
  ...
  async create(priceListId: string, input: CreatePriceListItemInput): Promise<PriceListItem> {
    await this.ensureEditablePriceListExists(priceListId);
  ...
  private async ensureEditablePriceListExists(priceListId: string, requireDraft = true): Promise<void> {
    ...
    if (requireDraft && priceList.status !== 'draft') {
      throw new ConflictException({ code: 'INVALID_TRANSITION', message: 'Only draft price lists can be changed' });
    }
  }
}
