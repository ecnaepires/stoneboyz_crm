...
> Detach, reassignment, and release-to-shop-stock semantics per ownership are fixed in [ADR 0005](../../adr/0005-restricted-material-release-and-reassign.md): plain detach is blocked for restricted material, reassign blocks customer-supplied across customers, and every ownership/reservation change writes a `slab_audit_events` row.
