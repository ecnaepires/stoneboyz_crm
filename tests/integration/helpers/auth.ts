...
export async function seedTestSession(pool: Pool, role = 'admin'): Promise<string> {
  ...
  await pool.query(
    `INSERT INTO "user" (id, name, email, "emailVerified", "createdAt", "updatedAt", role)
     VALUES ($1, 'Test Actor', 'test@stoneboyz.test', true, now(), now(), $2)
     ON CONFLICT (id) DO UPDATE SET role = $2`,
    [TEST_ACTOR_USER_ID, role]
  );
